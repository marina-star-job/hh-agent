...
TITLE_BLACKLIST = [
    ...
    # Рядовые грейды — кандидат руководитель, не исполнитель:
    r"\bстарший\s+менеджер\b",       # "Старший менеджер по работе с Ozon"
    r"\bрегиональный\s+менеджер\b",  # региональный менеджер
    r"\bзаместитель\s+(руководител|роп)",  # заместитель руководителя / заместитель РОП
    r"\bзам\.\s*руководител",              # зам. руководителя
    r"\bменеджер\s+по\s+работе\s+с\s+клиентами\b",  # клиентский менеджер (исполнитель)
    # "менеджер по продажам" БЕЗ слов руководитель/head/начальник/директор рядом:
    r"^(?!.*\b(?:руководител|начальник|директор|head)).*\bменеджер\s+по\s+продажам\b",
    # Ловушки "рядовой с ростом в руководителя":
    r"\bрост\w*\s+(в|до)\s+(руководител|роп)",  # "с ростом в руководителя", "рост до руководителя отдела"
    r"\bроста\s+до\s+(руководител|роп)",        # "с перспективой роста до руководителя/РОП"
]
...
Шаг 2. Hard NO признаки → decision="no", score=0-2:
  ...
  - ИНОСТРАННЫЙ ЯЗЫК КРОМЕ АНГЛИЙСКОГО → hard NO. Если в вакансии требуется знание
    иностранного языка, кроме английского (словацкий, немецкий, французский,
    китайский, испанский и любой другой) → decision="no". Кандидат знает английский
    (B1), другими языками не владеет. Английский как требование — ОК, НЕ режем.
...
Пример 9:
Вакансия: "Менеджер со знанием словацкого языка"
Ответ: {"decision": "no", "match_score": 1, "tier": "out_of_scope", "concerns": ["Иностранный язык кроме английского"], "reason": "Требуется знание словацкого языка - кандидат владеет только английским, hard NO."}
...
Напиши сопроводительное письмо."""
    ...
    # Ссылка на резюме — в конец КАЖДОГО письма, после подписи.
    # Добавляем ПОСЛЕ пост-обработки тире: в URL дефисов нет, но так replace
    # гарантированно не затрагивает ссылку.
    letter = letter.rstrip() + "\n\nРезюме: https://alexotkidach.netlify.app/"
